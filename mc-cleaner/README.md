...
- `MC-Cleaner-GUI-portable.zip`: 可视化版本，支持选择存档文件夹和自由填写删除阈值。
...
下载 `releases/MC-Cleaner-GUI-portable.zip`，解压后运行文件夹里的 `MC-Cleaner-GUI.exe`：
...
GUI 版会生成 portable zip，Lite 版会生成单 exe，发布文件放在 `releases/`。
...
- `MC-Cleaner-GUI-portable.zip`: GUI version with save-folder selection and a configurable cleanup threshold.
...
Download `releases/MC-Cleaner-GUI-portable.zip`, extract it, then run `MC-Cleaner-GUI.exe` inside the extracted folder:
...
The GUI version is packaged as a portable zip, the Lite version is packaged as a single exe, and release files are copied to `releases/`.
